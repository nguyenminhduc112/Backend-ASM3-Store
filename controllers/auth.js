                ...
                bcrypt.compare(password, user.password, function (err, result) {
                    if (err) {
                        ...
                        if (user.admin) {
                            ...
                            res.status(400).json({ message: "You're are not admin" })
                        }
                    }
                });
            } else {
...
exports.postSignup = async (req, res, next) => {
    try {
        ...
        if (userFind) {
            ...
            const user = await User({ name: name, email: email, password: hasPassword, cart: { items: [] } })
            await user.save()
            res.status(201).json({ message: 'User Created Successly!', })
        }
